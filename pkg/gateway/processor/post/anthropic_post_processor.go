...
func init() {
	registry.RegisterPostProcessor("anthropic", func(params map[string]interface{}) registry.PostProcessor {
		return newAnthropicPostProcessor()
	})
}

// AnthropicPostProcessor is a post-processor that converts OpenAI streaming responses to Anthropic format.
...
// newAnthropicPostProcessor creates and returns a new AnthropicPostProcessor instance.
func newAnthropicPostProcessor() *AnthropicPostProcessor {
	return &AnthropicPostProcessor{}
}

// Name returns the name of the processor.
func (c *AnthropicPostProcessor) Name() string {
	return "anthropic_post_processor"
}
...
func (c *AnthropicPostProcessor) PostStreamProcess(req *types.RequestContext, done bool) error {
	...
	modifiedData, err := anthropic.ConvertOpenAIStreamingResponseToAnthropic(
		oaiStreamResp,
		isFirst,
		done,
		req.Id,
...
func (c *AnthropicPostProcessor) PostProcess(req *types.RequestContext) error {
	// Extract request context and OpenAI streaming response
	anthropicReq := req.AnthropicRequest
	oaiResp := anthropicReq.OpenAIResponse

	anthropicResp, err := anthropic.ConvertOpenAIResponseToAnthropic(oaiResp, req.Id, anthropicReq.Request.Model)
	if err != nil {
