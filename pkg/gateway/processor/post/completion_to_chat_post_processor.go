...
func init() {
	registry.RegisterPostProcessor("completion_to_chat", func(params map[string]interface{}) registry.PostProcessor {
		config, _ := params["config"].(*options.Config)
		return newCompletionToChatPostProcessor(config)
	})
}
...
type CompletionToChatPostProcessor struct {
...
func newCompletionToChatPostProcessor(config *options.Config) *CompletionToChatPostProcessor {
	...
	return &CompletionToChatPostProcessor{
		tokenizer:       tk,
...
func (rp *CompletionToChatPostProcessor) Name() string {
	return "completion_to_chat_post_processor"
}

func (rp *CompletionToChatPostProcessor) ChatCompletionStreamProcess(req *types.RequestContext, done bool) error {
	...
	llmRequest.ReasoningTokensLen += reasoningTokensLen
	capacity := llmRequest.MaxTokensLimit - (llmRequest.OutputTokensLen - llmRequest.ReasoningTokensLen)
	...
	if rawContentTokensLen >= capacity {
		...
		llmRequest.OutputTokensLen += (reasoningTokensLen + capacity)
	} else {
		// if contentTokensLen < capacity, it means the content is not truncated, so we keep the finish reason from backend
		llmRequest.OutputTokensLen += (reasoningTokensLen + rawContentTokensLen)
		if parseResult != nil && len(parseResult.ToolCalls) > 0 {
			llmRequest.HasToolCalls = true
			toolCalls, _ := json.Marshal(parseResult.ToolCalls)
			llmRequest.OutputTokensLen += getTokenLen(string(toolCalls))
		}
	}
	klog.V(3).Infof("[%s] ChatCompletionStreamProcess: output_len: %d, reasoning_content_len: %d, raw_content_len: %d, capacity: %d, max_len: %d",
		req.Id, llmRequest.OutputTokensLen, llmRequest.ReasoningTokensLen, rawContentTokensLen, capacity, llmRequest.MaxTokensLimit)

	needSplitLastResp := false
	if choice.FinishReason != "" {
		if choice.Text != "" {
			needSplitLastResp = true
		} else if llmRequest.HasToolCalls && choice.FinishReason == "stop" {
	...
	chatStreamResp.Usage = &protocol.Usage{
		PromptTokens:     llmRequest.InputTokensLen,
		TotalTokens:      llmRequest.InputTokensLen + llmRequest.OutputTokensLen,
		CompletionTokens: llmRequest.OutputTokensLen,
		CompletionTokensDetails: &protocol.CompletionTokensDetails{
			ReasoningTokens: llmRequest.ReasoningTokensLen,
		},
	}
...
func (rp *CompletionToChatPostProcessor) ChatCompletionProcess(req *types.RequestContext, done bool) error {
	llmRequest := req.LLMRequest
	if done {
...
func (rp *CompletionToChatPostProcessor) completionStreamProcess(req *types.RequestContext, done bool) error {
	...
	for _, choice := range cmplResp.Choices {
		if choice.Text != "" {
			...
			req.LLMRequest.OutputTokensLen += uint64(len(tokens))
		}
	}
	return nil
}

func (rp *CompletionToChatPostProcessor) completionProcess(req *types.RequestContext, done bool) error {
	if done {
...
func (rp *CompletionToChatPostProcessor) trySetParser(req *types.RequestContext) error {
...
func (rp *CompletionToChatPostProcessor) PostStreamProcess(req *types.RequestContext, done bool) error {
...
func (rp *CompletionToChatPostProcessor) PostProcess(req *types.RequestContext) error {
	return nil
}
