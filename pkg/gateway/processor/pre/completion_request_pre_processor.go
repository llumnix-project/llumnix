...
func init() {
	registry.RegisterPreProcessor("completion_request", func(params map[string]interface{}) registry.PreProcessor {
		return newCompletionRequestPreProcessor()
	})
}

const defaultMaxTokens = 16384

type CompletionRequestPreProcessor struct{}

func newCompletionRequestPreProcessor() *CompletionRequestPreProcessor {
	return &CompletionRequestPreProcessor{}
}

func (rt *CompletionRequestPreProcessor) Name() string {
	return "completion_request_pre_processor"
}

func (rt *CompletionRequestPreProcessor) TokenizerEncode(prompt string, addSpecialTokens bool) ([]uint32, error) {
	tk, _ := tokenizer.GetTokenizer()
	return tk.Encode(prompt, addSpecialTokens)
}

func (rt *CompletionRequestPreProcessor) applyTokenizerTemplate(req *types.RequestContext) ([]uint32, error) {
	tk, _ := tokenizer.GetTokenizer()
...
func (rt *CompletionRequestPreProcessor) generateMaxTokens(req *types.RequestContext, maxTokens *int) int {
...
func (rt *CompletionRequestPreProcessor) PreProcess(req *types.RequestContext) error {
