...
func TestLocalRealtimeState(t *testing.T) {
	...
	t.Run("Add Gateway", func(t *testing.T) {
		lrs.AddGateway(testGatewayId)
		assert.True(t, lrs.gatewayExists(testGatewayId))
	})
	...
	t.Run("Remove Gateway", func(t *testing.T) {
		lrs.RemoveGateway(testGatewayId)
		assert.False(t, lrs.gatewayExists(testGatewayId))
	})
}
...
func TestMetrics(t *testing.T) {
	...
	assert.Equal(t, int64(300), instanceViews[instance.Id()].NumTokens())
	assert.Equal(t, int64(2), instanceViews[instance.Id()].NumRequests())
...
// lrs Not support concurrent operations yet
...
func TestEdgeCases(t *testing.T) {
	...
	t.Run("Instance Version Change", func(t *testing.T) {
		...
		// Try to update - should fail because request was cleaned up with old instance
...
func TestGetAllWorkStatsByModel(t *testing.T) {
	...
	t.Run("Single Model Match", func(t *testing.T) {
		...
		assert.Equal(t, gpt35Instance1.Id(), results[gpt35Instance1.Id()].GetInstance().Id())
	})
	...
	t.Run("Different Models", func(t *testing.T) {
		...
		assert.Equal(t, gpt4Instance.Id(), gpt4Results[gpt4Instance.Id()].GetInstance().Id())

		// Test Claude model
		claudeResults := lrs.GetInstanceViewsByModel("claude-2")
		assert.Equal(t, 1, len(claudeResults))
		assert.Equal(t, claudeInstance.Id(), claudeResults[claudeInstance.Id()].GetInstance().Id())
	...
	t.Run("Mixed Models With States", func(t *testing.T) {
		...
		gpt4Results := lrs.GetInstanceViewsByModel("gpt-4")
		assert.Equal(t, 1, len(gpt4Results))
		assert.Equal(t, int64(200), gpt4Results[gpt4Instance.Id()].NumTokens())
	})
