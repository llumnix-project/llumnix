...
if TYPE_CHECKING:
    ...
    LLUMNIX_PUSH_CMS_INTERVAL: float = 0.04
    LLUMNIX_ENGINE_GET_STATUS_TIMEOUT: float = 60.0
    ...
    LLUMNIX_LOG_INSTANCE_STATUS_INTERVAL_S: float = 10.0
    LLUMNIX_RECENT_WAITINGS_STALENESS_SECONDS: float = 10.0
    LLUMNIX_ENABLE_MIGRATION: int = 0
...
environment_variables: Dict[str, Callable[[], Any]] = {
    ...
    # if set, llumnix will push instance status to cms with specific interval
    "LLUMNIX_PUSH_CMS_INTERVAL": lambda: float(os.getenv("LLUMNIX_PUSH_CMS_INTERVAL", "0.04")),
    # this is used for configuring the timeout for get_instance_status engine call
    "LLUMNIX_ENGINE_GET_STATUS_TIMEOUT": lambda: float(os.getenv("LLUMNIX_ENGINE_GET_STATUS_TIMEOUT", "60.0")),
    ...
    # this is used for setting the interval of logging instance status
    "LLUMNIX_LOG_INSTANCE_STATUS_INTERVAL_S": lambda: float(os.getenv("LLUMNIX_LOG_INSTANCE_STATUS_INTERVAL_S", "10.0")),
    ...
    "LLUMNIX_ENABLE_MIGRATION": lambda: int(os.getenv("LLUMNIX_ENABLE_MIGRATION", "0")),
    ...
    # Metric names should match the gateway's metric names and multiple metrics should be separated by comma.
