...
if envs.LLUMNIX_ENABLE_MIGRATION:
    try:
        ...
        raise ImportError(
            "LLUMNIX_ENABLE_MIGRATION is set to True, but failed to import from 'mooncake'. "
...
class MooncakeMigrationFrontend(BaseMigrationFrontend):
    def __init__(self, vllm_config: VllmConfig, dp_rank: int, scheduler: "Scheduler"):
        ...
        max_migrate_in_kv_cache_usage_ratio = (
            envs.LLUMNIX_MAX_KV_CACHE_USAGE_RATIO_MIG_IN
        )
        total_tokens = (
            self._cfg.cache_config.num_gpu_blocks * self._cfg.cache_config.block_size
        )
        ...
        self.migrate_thread = threading.Thread(
            target=self._call_remote_migrate_in_worker, name="MigrationWorker"
        )
    ...
    def _set_migrating_status(self, req: Request):
        ...
        req.kv_transfer_params["remote_host"] = (
            self.scheduler.connector.connector_scheduler.side_channel_host
        )
        req.kv_transfer_params["remote_port"] = (
            self.scheduler.connector.connector_scheduler.side_channel_port
        )

    def _get_sorted_running_indices(
        self, mig_policy: RequestMigrationPolicy, reqs: List[Request]
    ) -> list[int]:
        return self._get_sorted_indices_helper(
            mig_policy, reqs, lambda i: (reqs[i].num_tokens + reqs[i].num_output_tokens)
        )
    ...
    def update_migration_status(self, instance_status: InstanceStatus):
        ...
        if self.get_detailed_migration_status:
            with g_migrate_in_req_info_lock:
                instance_status.num_migrate_in_tokens = sum(
                    g_migrate_in_tokens.values()
                )
            with g_migrate_out_req_info_lock:
                instance_status.num_migrate_out_tokens = sum(
                    g_migrate_out_tokens.values()
                )
            instance_status.kv_cache_usage_ratio_migrate_in = round(
                instance_status.num_migrate_in_tokens
                / instance_status.num_total_gpu_tokens,
                4,
            )
            instance_status.kv_cache_usage_ratio_migrate_out = round(
    ...
    def _call_remote_migrate_in_worker(self):
        while not self._is_shutdown:
            try:
                ...
                self._execute_remote_migrate_in_task(
                    migrated_out_requests, dst_engine_address
                )
            except queue.Empty:
                continue
            except Exception as e:  # pylint: disable=broad-except
                logger.exception(
                    "An unexpected error occurred in the migration worker: %s", e
                )
    ...
    def migrate_out(
        self, migration_params: dict, dst_engine_host: str, dst_engine_port: int
    ) -> bool:
        ...
        if not migrated_out_requests:
            logger.warning(
                "No requests to migrate based on policy, migration type: %s",
                migration_params.migration_type,
            )
            return False
        dst_engine_address = f"{dst_engine_host}:{dst_engine_port}"
        self.scheduler.connector.migrate_out_request(
            migrated_out_requests, dst_engine_address
        )
    ...
    def call_remote_migrate_in(
        self, migrated_out_requests: List[Request], dst_engine_address: str
    ):
        """Puts a remote migration task into the queue."""
        task = (migrated_out_requests, dst_engine_address)
        self.remote_call_queue.put(task)

    def _execute_remote_migrate_in_task(
        self, migrated_out_requests: List[Request], dst_engine_address: str
    ):
    ...
    def _call_remote_migrate_in(
        self, dst_engine_address: str, serialized_migrate_req: bytes
    ):
        with grpc.insecure_channel(dst_engine_address) as channel:
            ...
            try:
                ...
                raise RuntimeError(
                    f"An RPC error occurred when sending migrate in req: {e.code()} - {e.details()}"
                ) from e
    ...
    def migrate_in(self, serialized_data: bytes) -> bool:
        ...
        if not self._has_migrate_in_slots(migrate_in_requests):
            logger.error(
                "Failed to migrate in. No enough slots for migrate in %s reqs",
                len(migrate_in_requests),
            )
            return False
        logger.info(
            "Migrating in requests: %s", [req.request_id for req in migrate_in_requests]
        )
