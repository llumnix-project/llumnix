...
from llumnix.instance_info import BackendType, ConnectorType, InstanceMetaData, InstanceStatus
...
class Llumlet:
    ...
    def add_llumlet_addresses(self, addresses: Optional[dict[str, str]]) -> list[str]:
        ...
        addresses = add_llumlet_addresses(self.engine_type, addresses, self.client_addresses)
        return addresses
    ...
    def terminate(self):
        if self.proc.is_alive():
            logger.info("Signaling Llumlet process (PID: %s) to shutdown...", self.proc.pid)
            self.proc.terminate()
...
class LlumletProc:
    ...
    class RpcHandler:
        ...
        async def abort_migrate_in(self, request_id: str, migrate_in_ip_address: str | None, migrate_in_port: str | None) -> None:
            await self.llumlet.abort_migrate_in(request_id, migrate_in_ip_address, migrate_in_port)
    ...
    def run_llumlet(
        ...
        try:
            ...
            raise ValueError(
                "Invalid engine_type: {}. Valid options: {}".format(engine_type, [e.value for e in BackendType])
            ) from e
        try:
            llumlet = LlumletProc(
                engine_type=engine_type,
    ...
    async def _execute_with_timeout(
        ...
        for attempt in range(max_retries):
            ...
            try:
                ...
                logger.warning("Operation '{}' timeout (attempt {}/{}), retry in {} seconds.".format(
                    operation_name, attempt+1, max_retries, delay))
                if attempt < max_retries - 1:
                    await asyncio.sleep(delay)
                last_exception = e
            except Exception as e:  # pylint: disable=broad-except
                logger.exception("Error during %s on attempt %s/%s: %s",
                                 operation_name, attempt + 1, max_retries, e)
                if attempt < max_retries - 1:
                    await asyncio.sleep(delay)
                last_exception = e
        logger.error("Operation '{}' failed after {} attempts, last error: {}".format(
    ...
    async def register_with_cms(self, max_retries=3, delay=5) -> bool:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    await asyncio.sleep(delay)
        logger.error("Could not register instance with CMS after %s attempts.", max_retries)
        raise RuntimeError("Failed to register with CMS after multiple retries") from last_exception
    ...
    async def get_instance_status_from_engine(self) -> InstanceStatus:
        ...
        success, instance_status_dict = await self._execute_with_timeout(
            self.engine_client.get_instance_status, "get_instance_status", envs.LLUMNIX_ENGINE_GET_STATUS_TIMEOUT,
        )
        if success:
    ...
    async def get_instance_status_from_queue(self) -> InstanceStatus:
        ...
        try:
            instance_status = await asyncio.wait_for(self._get_instance_status_from_queue(), envs.LLUMNIX_ENGINE_GET_STATUS_TIMEOUT)
            self.complete_instance_status(instance_status)
    ...
    def complete_instance_status(self, instance_status: CMSInstanceStatus) -> None:
        ...
        if self.enable_mig:
            instance_status.num_available_migrate_in_slots = self.mig_limits.max_req_mig_in - instance_status.num_migrate_in_reqs
            instance_status.num_available_migrate_out_slots = self.mig_limits.max_req_mig_out - instance_status.num_migrate_out_reqs
    ...
    async def abort(self, request_ids: List[RequestIDType]) -> None:
        success, _ = await self._execute_with_timeout(
            lambda: self.engine_client.abort(request_ids), "abort", envs.LLUMNIX_ENGINE_ABORT_TIMEOUT
        )
        if not success:
            raise RuntimeError("Engine client failed to abort requests {}".format(request_ids))

    async def migrate_in(self, serialized_migrate_req: bytes, serialization_format: str) -> bool:
        """Process migration in request with serialized data"""
        success, res = await self._execute_with_timeout(
            lambda: self.engine_client.migrate_in(serialized_migrate_req, serialization_format),
            "migrate_in",
    ...
    async def abort_migrate_in(self, request_ids: list[str], migrate_in_ip_address: str | None, migrate_in_port: str | None) -> None:
        ...
        if self.engine_type == BackendType.SGLANG:
            for req_id in request_ids:
                ...
                if not success:
                    raise RuntimeError("Engine client failed to process AbortMigrateIn request for {}".format(req_id))
        else:
            success, _ = await self._execute_with_timeout(
            ...
            if not success:
                raise RuntimeError("Engine client failed to process AbortMigrateIn request for {}".format(request_ids))
