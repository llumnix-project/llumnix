...
class InstanceStatusCollector:
    def __init__(
        ...
        self.connector_update_collector = self.create_connector_status_collector(
            connector_type, scheduler
        )
        self.instance_status_mask = generate_instance_status_mask(
            parse_used_metrics_from_env()
        )
        logger.debug("instance_status_mask: {}".format(self.instance_status_mask))

    def create_connector_status_collector(
        self, connector_type: ConnectorType, scheduler: "Scheduler"
    ) -> BaseConnectorStatusCollector:
        if not connector_type:
            return None
        if connector_type == ConnectorType.MOONCAKE:
            # pylint: disable=import-outside-toplevel
            from llumnix.status_collector.vllm_v1.mooncake_connector_status_collector import (
                MooncakeConnectorStatusCollector,
            )

            return MooncakeConnectorStatusCollector(scheduler)
        if connector_type == ConnectorType.HYBRID:
            # pylint: disable=import-outside-toplevel
            from llumnix.status_collector.vllm_v1.hybrid_connector_status_collector import (
                HybridConnectorStatusCollector,
            )
    ...
    def collect_by_name(
        self, status_name: str, instance_status: InstanceStatus, *args, **kwargs
    ):
    ...
    def get_waiting_status(
        self,
        waiting_status: List[str],
        instance_status: InstanceStatus,
        all_waitings: List[Request],
    ) -> None:
        for status_name in waiting_status:
            if self.instance_status_mask.get(status_name, False):
                if status_name in _REGISTERED_STATUS_COLLECTORS:
                    ...
                    logger.warning(
                        "Status {} collect function is not found.".format(status_name)
                    )
                ...
                if status_name in _REGISTERED_STATUS_COLLECTORS:
                    if status_name in [
                        ...
                        self.collect_by_name(
                            status_name, instance_status, step_phase, scheduler_output
                        )
                    else:
                        self.collect_by_name(status_name, instance_status)
                else:
    ...
    def get_all_waiting_reqs(self) -> List[Request]:
        ...
        if self.connector_update_collector:
            all_waitings.extend(
                self.connector_update_collector.get_connector_waiting_reqs()
            )
        return all_waitings

    @register("hybrid_scheduler_waiting_to_decode_requests_num")
    def _collect_hybrid_scheduler_waiting_to_decode_requests_num(
        self, instance_status: InstanceStatus
    ) -> None:
        if not self.connector_update_collector:
            return
        instance_status.hybrid_scheduler_waiting_to_decode_requests_num = (
            self.connector_update_collector.get_connector_waiting_to_decode_reqs_num()
        )

    @register("num_uncomputed_tokens_hybrid_scheduler_waiting_prefills")
    def _collect_num_uncomputed_tokens_hybrid_scheduler_waiting_prefills(
        self, instance_status: InstanceStatus
    ) -> int:
        if not self.connector_update_collector:
            return 0
        ...
        instance_status.num_uncomputed_tokens_hybrid_scheduler_waiting_prefills = (
            num_uncomputed_tokens_hybrid_scheduler_waiting_prefills
        )
    ...
    def _collect_num_unallocated_tokens_hybrid_scheduler_waiting_decodes(
        self, instance_status: InstanceStatus
    ) -> None:
        if not self.connector_update_collector:
            return
    ...
    def _collect_hybrid_scheduler_waiting_to_decode_tokens_num(
        self, instance_status: InstanceStatus
    ) -> None:
        if not self.connector_update_collector:
            return
        instance_status.hybrid_scheduler_waiting_to_decode_tokens_num = (
            self.connector_update_collector.get_connector_waiting_to_decode_tokens_num()
        )

    @register("num_uncomputed_tokens_scheduler_waiting_prefills")
    def _collect_num_uncomputed_tokens_scheduler_waiting_prefills(
        self, instance_status: InstanceStatus
    ) -> int:
        ...
        instance_status.num_uncomputed_tokens_scheduler_waiting_prefills = (
            num_uncomputed_tokens_scheduler_waiting_prefills
        )
    ...
    def _collect_scheduler_waiting_to_decode_requests_num(
        self, instance_status: InstanceStatus
    ) -> None:
        scheduler_waiting_to_decode_requests_num = 0
        for req in self.scheduler.waiting:
            if (
                req.sampling_params is not None and req.sampling_params.max_tokens > 1
            ) or req.num_tokens - req.num_computed_tokens == 1:
                scheduler_waiting_to_decode_requests_num += 1
        instance_status.scheduler_waiting_to_decode_requests_num = (
            scheduler_waiting_to_decode_requests_num
        )

    @register("scheduler_waiting_to_decode_tokens_num")
    def _collect_scheduler_waiting_to_decode_tokens_num(
        self, instance_status: InstanceStatus
    ) -> None:
        ...
        instance_status.scheduler_waiting_to_decode_tokens_num = (
            scheduler_waiting_to_decode_tokens_num
        )

    @register("num_uncomputed_tokens_all_waiting_prefills")
    def _collect_num_uncomputed_tokens_all_waiting_prefills(
        self, instance_status: InstanceStatus
    ) -> None:
        num_uncomputed_tokens_hybrid_scheduler_waiting_prefills = (
            self._collect_num_uncomputed_tokens_hybrid_scheduler_waiting_prefills(
                instance_status
            )
        )
        num_uncomputed_tokens_scheduler_waiting_prefills = (
            self._collect_num_uncomputed_tokens_scheduler_waiting_prefills(
                instance_status
            )
        )
        ...
        instance_status.num_uncomputed_tokens_all_waiting_prefills = (
            num_uncomputed_tokens_all_waiting_prefills
        )

    @register("num_waiting_requests")
    def _collect_num_waiting_requests(
        self, instance_status: InstanceStatus, all_waitings: List[Request]
    ) -> None:
    ...
    def _collect_num_uncomputed_tokens_scheduler_running_prefills(
                ...
                if req.num_computed_tokens < req.num_prompt_tokens:
                    ...
                    if (
                        ...
                        if req.request_id in scheduler_output.num_scheduled_tokens:
                            ...
                            if (
                                scheduler_output.num_scheduled_tokens[req.request_id]
                                > 1
                            ):
                    ...
                    if (
                        ...
                        and self.last_num_scheduled_tokens is not None
                    ):
                        # exclude request send from p to d
                        if (
                            req.request_id in self.last_num_scheduled_tokens
                            and self.last_num_scheduled_tokens[req.request_id] > 1
                        ):
                            num_uncomputed_tokens += self.last_num_scheduled_tokens[
                                req.request_id
                            ]
                    num_uncomputed_tokens_scheduler_running_prefills += (
                        num_uncomputed_tokens
        ...
        instance_status.num_uncomputed_tokens_scheduler_running_prefills = (
            num_uncomputed_tokens_scheduler_running_prefills
        )

    @register("num_unallocated_tokens_scheduler_running_prefills")
    def _collect_num_unallocated_tokens_scheduler_running_prefills(
        self, instance_status: InstanceStatus
    ) -> None:
            ...
            for req in self.scheduler.running:
                if req.num_computed_tokens < req.num_prompt_tokens:
                    ...
                    num_unallocated_tokens_scheduler_running_prefills += (
                        num_unallocated_tokens
                    )
        instance_status.num_unallocated_tokens_scheduler_running_prefills = (
            num_unallocated_tokens_scheduler_running_prefills
        )

    @register("scheduler_running_to_decode_requests_num")
    def _collect_scheduler_running_to_decode_requests_num(
        self, instance_status: InstanceStatus
    ) -> None:
        ...
        if self.scheduler.running:
            for req in self.scheduler.running:
                if (
                    ...
                    and req.sampling_params.max_tokens > 1
                ):
                    if (
                        self.connector_update_collector
                        and self.connector_update_collector.is_migrating(req)
                    ):
                        continue
                    scheduler_running_to_decode_requests_num += 1
        instance_status.scheduler_running_to_decode_requests_num = (
            scheduler_running_to_decode_requests_num
        )

    @register("scheduler_running_to_decode_tokens_num")
    def _collect_scheduler_running_to_decode_tokens_num(
        self, instance_status: InstanceStatus
    ) -> None:
        ...
        instance_status.scheduler_running_to_decode_tokens_num = (
    ...
    def _collect_num_loading_requests(self, instance_status: InstanceStatus) -> None:
        if self.connector_update_collector is None:
            return
        instance_status.num_loading_requests = (
            self.connector_update_collector.get_connector_loading_requests_num()
        )

    @register("num_tokens_loading_requests")
    def _collect_num_tokens_loading_requests(
        self, instance_status: InstanceStatus
    ) -> None:
        if self.connector_update_collector is None:
            return
