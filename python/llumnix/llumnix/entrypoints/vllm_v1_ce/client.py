...
class VLLMV1DPLlumnixClient(LlumnixClient, DPAsyncMPClient):
    def __init__(
        self,
        loop: asyncio.AbstractEventLoop,
        output_processor: OutputProcessor,
    ...
    async def add_request_async(self, request: EngineCoreRequest) -> None:
        ...
        if not self.engines_running:
            ...
            if not VLLM_VERSION.startswith("0.9"):
                # In vllm 0.10+, encode req message
...
class VLLMV1DPLBLlumnixClient(LlumnixClient, DPLBAsyncMPClient):
    def __init__(
        self,
        loop: asyncio.AbstractEventLoop,
        output_processor: OutputProcessor,
