...
class AsyncLlumletRPCServer:
    ...
    async def start(self, handler: Any) -> None:
        ...
        llumlet_server_pb2_grpc.add_LlumletServicer_to_server(
            self.AsyncServicer(handler), self.server
        )
        bound_port = self.server.add_insecure_port(f"[::]:{self.port}")
        logger.info(
            "Server port configured. Attempting to bind to port %s...", bound_port
        )
        await self.server.start()
        logger.info(
            "Server started successfully and is listening on port %s.", bound_port
        )
        return bound_port
    ...
    class AsyncServicer(llumlet_server_pb2_grpc.LlumletServicer):
        ...
        async def Migrate(
            ...
            try:
                ...
                context.set_details(
                    "Migration failed due to invalid argument: {}".format(e)
                )
                return llumlet_server_pb2.MigrateResponse(
                    message="Migration failed due to invalid argument {}.".format(
                        str(e)
                    ),
                    success=False,
                )
            try:
                ...
                res = await asyncio.wait_for(
                    self.handler.migrate(dst_engine_ip, dst_engine_port, mig_params),
                    timeout=self.timeout,
                )
                if res:
                    return llumlet_server_pb2.MigrateResponse(
                        message="Migrate requests from {} to {}.".format(
                            request.src_engine_id, request.dst_engine_id
                        ),
                        success=True,
                    )
                return llumlet_server_pb2.MigrateResponse(
                ...
                logger.warning(
                    "Migration request timed out after %s seconds.", self.timeout
                )
                context.set_code(grpc.StatusCode.DEADLINE_EXCEEDED)
                context.set_details(
                    "Migration failed to complete within the server's timeout of {} seconds.".format(
                        self.timeout
                    )
                )
                return llumlet_server_pb2.MigrateResponse(
                    message="Migration failed due to timeout {}.".format(str(e)),
                    success=False,
                )
                ...
                return llumlet_server_pb2.MigrateResponse(
                    message=f"No enough migrate out slots. {str(e)}.", success=False
                )
            except Exception as e:  # pylint: disable=broad-except
                logger.error("Migration failed: %s", e, exc_info=True)
                context.set_code(grpc.StatusCode.INTERNAL)
                context.set_details(
                    "Migration failed due to an unexpected server error: {}".format(e)
                )
                return llumlet_server_pb2.MigrateResponse(
                    message="Migration failed due to unexpected exception {}.".format(
                        str(e)
                    ),
                    success=False,
                )
        ...
        async def Abort(
            ...
            try:
                await asyncio.wait_for(
                    self.handler.abort(request_ids), timeout=self.timeout
                )
                ...
                context.set_code(grpc.StatusCode.INTERNAL)
                context.set_details(
                    "Failed to abort due to an unexpected server error: {}".format(e)
                )
                return llumlet_server_pb2.AbortResponse(
                    success=False,
                    message="Failed to abort due to unexpected exception {}.".format(
                        str(e)
                    ),
                )
        ...
        async def MigrateIn(
            ...
            if serialization_format:
                logger.info(
                    "Received MigrateIn request with format: %s", serialization_format
                )
            try:
                res = await asyncio.wait_for(
                    self.handler.migrate_in(
                        request.serialized_migrate_req, serialization_format
                    ),
                    timeout=self.timeout,
                )
                return llumlet_server_pb2.MigrateInResponse(
                    success=res, message="MigrateIn request processed successfully."
                )
            except asyncio.TimeoutError as e:
                logger.warning(
                    "MigrateIn request timed out after %s seconds.", self.timeout
                )
                context.set_code(grpc.StatusCode.DEADLINE_EXCEEDED)
                context.set_details(
                    "Failed to process MigrateIn within the server's timeout of {} seconds.".format(
                        self.timeout
                    )
                )
                return llumlet_server_pb2.MigrateInResponse(
                ...
                context.set_code(grpc.StatusCode.INTERNAL)
                context.set_details(
                    "MigrateIn failed due to an unexpected server error: {}".format(e)
                )
                return llumlet_server_pb2.MigrateInResponse(
                    success=False,
                    message="MigrateIn failed due to unexpected exception {}.".format(
                        str(e)
                    ),
                )
        ...
        async def AbortMigrateIn(
            ...
            if request.has_field("migrate_in_ip_address") and request.has_field(
                "migrate_in_port"
            ):
                migrate_in_ip_address = request.migrate_in_ip_address
                migrate_in_port = request.migrate_in_port
                logger.info(
                    "Received AbortMigrateIn request for request_ids: %s", request_ids
                )
            else:
            ...
            try:
                await asyncio.wait_for(
                    self.handler.abort_migrate_in(
                        request_ids, migrate_in_ip_address, migrate_in_port
                    ),
                    timeout=self.timeout,
                )
                return llumlet_server_pb2.AbortResponse(
                    success=True,
                    message="AbortMigrateIn request processed successfully. requests ids: {}".format(
                        request_ids
                    ),
                )
            except asyncio.TimeoutError as e:
                logger.warning(
                    "AbortMigrateIn request timed out after %s seconds.", self.timeout
                )
                context.set_code(grpc.StatusCode.DEADLINE_EXCEEDED)
                context.set_details(
                    "Failed to process AbortMigrateIn within the server's timeout of {} seconds.".format(
                        self.timeout
                    )
                )
                return llumlet_server_pb2.AbortResponse(
                ...
                context.set_code(grpc.StatusCode.INTERNAL)
                context.set_details(
                    "AbortMigrateIn failed due to an unexpected server error: {}".format(
                        e
                    )
                )
                return llumlet_server_pb2.AbortResponse(
                    success=False,
                    message="AbortMigrateIn failed due to unexpected exception {}.".format(
                        str(e)
                    ),
                )
        ...
        async def PushInstanceStatus(
            ...
            try:
                ...
                return llumlet_server_pb2.PushInstanceStatusResponse(
                    success=True, message="Push instance status successfully."
                )
            # pylint: disable=broad-except
            except Exception as e:
                logger.error(
                    "Failed to process PushInstanceStatus: %s", e, exc_info=True
                )
                context.set_code(grpc.StatusCode.INTERNAL)
                context.set_details(
                    "PushInstanceStatus failed due to an unexpected server error: {}".format(
                        e
                    )
                )
                return llumlet_server_pb2.PushInstanceStatusResponse(
                    success=False,
                    message="PushInstanceStatus failed due to unexpected exception {}.".format(
                        str(e)
                    ),
                )
...
def validate_migration_type(type_string: str) -> None:
    ...
    try:
        ...
        logger.error(
            "Invalid migration type: %s. Must be one of: %s",
            type_string,
            [member.value for member in MigrationType],
        )
        raise ValueError(
            "'{}' is not a valid MigrationType.".format(type_string)
        ) from e


def generate_migration_params(request: llumlet_server_pb2.MigrateRequest):
    migration_params: MigrationParams = MigrationParams(
    ...
    if request.migration_type == MigrationType.NUM_REQ.value:
        if not request.HasField("num_reqs"):
            raise ValueError(
                "Empty num_reqs. If migration_type is NUM_REQ, num_reqs is necessary."
            )
    ...
    if request.migration_type == MigrationType.TOKEN.value:
        if not request.HasField("num_tokens"):
            raise ValueError(
                "Empty num_tokens. If migration_type is TOKEN, num_tokens is necessary."
            )
    ...
    if request.migration_type == MigrationType.RATIO.value:
        if not request.HasField("kv_cache_usage_ratio"):
            raise ValueError(
                "Empty kv_cache_usage_ratio. If migration_type is RATIO, kv_cache_usage_ratio is necessary."
            )
