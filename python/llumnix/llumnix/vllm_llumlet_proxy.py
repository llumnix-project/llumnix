...
def init_instance_status_with_scheduler(scheduler):
    num_total_gpu_tokens = (
        scheduler.cache_config.num_gpu_blocks * scheduler.cache_config.block_size
    )
...
class VLLMLlumletProxy:
    def __init__(self, vllm_config: VllmConfig, scheduler: "Scheduler", engine_index: int = 0, engine_type: str = "vLLM v1"):  # type: ignore
        ...
        if self.enable_migration:
            if not self.connector_type:
                logger.warning(
                    "Migration enabled but no kv_transfer_config found. Migration will be disabled."
                )
                self.enable_migration = False
                os.environ["LLUMNIX_ENABLE_MIGRATION"] = "0"
            elif self.connector_type == ConnectorType.HYBRID:
                # pylint: disable=import-outside-toplevel
                from llumnix.migration_frontend.vllm_v1.kvt_migration_frontend import (
                    KVTMigrationFrontend,
                )

                self.migration_frontend = KVTMigrationFrontend(
                    vllm_config=vllm_config,
                    dp_rank=vllm_config.parallel_config.data_parallel_rank,
                    scheduler=scheduler,
                )
                self.mig_async = True
                self.migration_frontend.mig_server_ready_event.wait(
                    timeout=MIGRATION_FRONTEND_INIT_TIMEOUT
                )
                self.mig_server_address = self.migration_frontend.mig_server.address
            elif self.connector_type == ConnectorType.MOONCAKE:
                # pylint: disable=import-outside-toplevel
                from llumnix.migration_frontend.vllm_v1.mooncake_migration_frontend import (
                    MooncakeMigrationFrontend,
                )

                self.migration_frontend = MooncakeMigrationFrontend(
                    vllm_config=vllm_config,
                    dp_rank=vllm_config.parallel_config.data_parallel_rank,
                    scheduler=scheduler,
                )
                ...
                raise NotImplementedError(
                    f"Failed to initialize migration frontend. kv_transfer_config:{vllm_config.kv_transfer_config}"
                )
        ...
        self.output_forwarder = ThreadOutputForwarder(
            vllm_config.instance_id, self.llumlet_grpc_address, engine_index
        )
    ...
    def add_abort_requests(
        self, request_queue: queue.Queue, request_ids: List[RequestIDType]
    ) -> None:
