...
class VLLMEngineClient(BaseEngineClient, AsyncMPClient):
                    ...
                    if output_handler is not None:
                        ...
                        if callable(output_handler):
                            await output_handler(
                                self_instance, outputs
                            )  # pylint: disable=not-callable
                        else:
                            logger.warning(
                                "output_handler is NOT callable. Type: %s. Skipping call.",
                                type(output_handler),
                            )
    ...
    async def migrate_out(
        ...
        res = await self.call_utility_async(
            "migrate_out", dst_engine_host, dst_engine_port, migration_params
        )
        return res.result
...
def _llumnix_process_utility_output(
    ...
    if future.cancelled():
        logger.warning(
            "llumnix utility future (call_id %s) is cancelled.", output.call_id
        )
        return
    if output.failure_message is not None:
        try:
            ...
            logger.error(
                "Failed to set exception on future (call_id: %s).", output.call_id
            )
    else:
        try:
            ...
            logger.error(
                "Failed to set result on future (call_id: %s).", output.call_id
            )
...
def gen_unit_id(vllm_config: VllmConfig) -> str:
    ...
    if not enable_dp:
        ...
        dp_master_ip_port = f"{parallel_config.data_parallel_master_ip}:{parallel_config.data_parallel_master_port}"
...
def vllm_get_instance_meta_data(cfg: VllmConfig) -> dict:
    ...
    if cfg.parallel_config.data_parallel_size:
        ...
        if cfg.parallel_config.data_parallel_size > 1:
            instance_id = (
                instance_id + "_dp" + str(cfg.parallel_config.data_parallel_rank)
            )
...
def vllm_get_connector_type(cfg: VllmConfig) -> str:
    ...
    raise ValueError(
        f"Unknown kv_connector type: {cfg.kv_transfer_config.kv_connector}"
    )
