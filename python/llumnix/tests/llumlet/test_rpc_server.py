...
class TestAsyncLlumletRPCServer(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_migrate_success(self):
        ...
        self.mock_handler.migrate.assert_awaited_once_with(
            "192.168.1.200",
            9999,
            MigrationParams(
                migration_type="NUM_REQ",
                mig_req_policy="SR",
                num_reqs=1,
                num_tokens=0,
        ...
        expected_message = "Migrate requests from %s to %s." % (
            src_engine_id,
            dst_engine_id,
        )
    ...
    async def test_migrate_failed_invalid_migration_type(self):
        ...
        self.assertIn(
            "'INVALID_TYPE' is not a valid MigrationType.", cm.exception.details()
        )
    ...
    async def test_migrate_failed_not_enough_slots(self):
        ...
        self.assertEqual(
            cm.exception.details(), "No enough migrate out slots for requests"
        )
        self.mock_handler.migrate.assert_awaited_once()
