...
class Heartbeat:
    ...
    def add_endpoint(self, host: str, port: int) -> None:
        ...
        if key not in self.endpoints:
            ...
            logger.info("Added endpoint: %s", key)
    ...
    def remove_endpoint(self, host: str, port: int) -> None:
        ...
        if key in self.endpoints:
            ...
            logger.info("Removed endpoint: %s", key)
    ...
    async def _check_endpoint(self, endpoint: EndpointStatus) -> bool:
        ...
        for attempt in range(self.config.retry_times):
            ...
            try:
                ...
                logger.warning(
                    "%s:%d - Connection error: %s (attempt %d/%d)",
                    endpoint.host,
                    endpoint.port,
                    e,
                    attempt + 1,
    ...
    async def _execute_loop_callbacks(self) -> None:
        ...
        for callback in self._loop_callbacks:
            try:
                ...
                logger.error("Loop callback error: %s", e)
    ...
    def print_status(self) -> None:
        print("\n" + "=" * 80)
        print(
            f"{'Endpoint':<25} {'Status':<12} {'Response Time':<15} {'Success Rate':<12} {'Error'}"
        )
        print("=" * 80)
        ...
        for key, endpoint in self.endpoints.items():
            ...
            response_time = (
                f"{endpoint.response_time}ms" if endpoint.response_time else "N/A"
            )
            error = endpoint.error_message or ""

            print(
                f"{key:<25} {endpoint.status.value:<12} {response_time:<15} {success_rate:<12} {error}"
            )
...
async def loop_completion_callback(endpoints: Dict[str, EndpointStatus]):
    ...
    healthy_count = sum(
        1 for ep in endpoints.values() if ep.status == HealthStatus.HEALTHY
    )
    total_count = len(endpoints)
    logger.info(
        "Heartbeat loop completed. Healthy endpoints: %d/%d", healthy_count, total_count
    )
...
def add_heartbeat_config_arguments(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        "--timeout", type=int, default=3, help="Request timeout in seconds (default: 5)"
    )
    parser.add_argument(
        "--retry-times",
        type=int,
        default=1,
        help="Retry attempts per check (default: 1)",
    )
    parser.add_argument(
        "--retry-interval",
        type=int,
        default=1,
        help="Interval between retries (default: 1)",
    )
    parser.add_argument(
        "--health-path",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Max consecutive failures (default: 1)",
    )


async def main():
    parser = argparse.ArgumentParser(
        description="Heartbeat monitor for checking endpoint health"
    )
    parser.add_argument(
        "--endpoints", nargs="+", required=True, help="Endpoints in format host:port"
    )
    ...
    for endpoint in args.endpoints:
        try:
            ...
            logger.error("Invalid endpoint format: %s. Expected: host:port", endpoint)
            return
