    ...
    async def _update_redis(self, *args, **kwargs) -> None:
        ...
        for instance_info in self.instance_infos:
            endpoint_status = self.heartbeat.get_status(
                instance_info.entrypoint_ip, instance_info.entrypoint_port
            )
        ...
        try:
            await self.redis_client.set(
                self.redis_key, pod_discovery_info.SerializeToString()
            )
            logger.info("Updated %d healthy instances in Redis", len(healthy_instances))
        except Exception as e:
            logger.error("Failed to update Redis: %s", e)
