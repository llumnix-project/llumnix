...
var testTokenizerPath string = "./test/data/sentence-transformers-labse.json"
...
func TestEmbeddingConfig(t *testing.T) {
	tk, err := tokenizers.FromFile(testTokenizerPath)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			ids := tk.Encode(tt.str, tt.addSpecial)
...
func TestMaxModelLen(t *testing.T) {
	tk, err := tokenizers.FromFile("./test/data/bert-base-uncased.json")
	require.NoError(t, err)
	defer tk.Close()
	assert.Equal(t, uint64(16384), tk.MaxModelLen())

	tk, err = tokenizers.FromFile("./test/data/Qwen/Qwen3-8B")
	require.NoError(t, err)
	defer tk.Close()
	assert.Equal(t, uint64(131072), tk.MaxModelLen())

}

func TestApplyChatTemplate(t *testing.T) {
	tk, err := tokenizers.FromFile("test/data/Qwen/Qwen3-Coder-480B-A35B-Instruct-FP8")
	// tk, err := tokenizers.FromFile("./test/data/Qwen/Qwen3-8B")
	require.NoError(t, err)
	defer tk.Close()

	messages := `[{"role":"system","content":"Youre a helpful assistant! Answer the users question best you can."},{"role":"user","content":"What is the weather like in Brooklyn, New York?"}]`
	...
	prompt, err := tk.ApplyChatTemplate(messages, tools, "")
	require.NoError(t, err)
...
func BenchmarkEncodeNTimes(b *testing.B) {
	...
	for _, tt := range tests {
		...
		b.Run(tt.name, func(b *testing.B) {
			b.ResetTimer()
			for i := 0; i < b.N; i++ {
				ids := tt.tk.Encode("brown fox jumps over the lazy dog", false)
...
func BenchmarkDecodeNTokens(b *testing.B) {
	...
	for _, tt := range tests {
		b.Run(tt.name, func(b *testing.B) {
			...
			validTokens := tt.tk.Encode(sampleText, false)
...
// func TestChatTemplateNullError(t *testing.T) {
// 	// template := `{% for message in messages %}{% if message.role == 'user' %}{{ 'User: ' + message.content }}{% else %}{{ 'Assistant: ' + message.content }}{% endif %}{% endfor %}`
// 	template := "test/data/deepseek-ai/DeepSeek-R1/tokenizer_config.json"
// 	ct, err := tokenizers.NewChatTemplate(template)
// 	if err != nil {
...
// 	file, _ := os.Open("test/data/nullerror/error.bin")

// 	defer file.Close()

// 	content, err := io.ReadAll(file)
// 	fmt.Printf("Read file content len: %v", len(content))
...
// 	result, err := ct.TestNull(messages_str)
// 	fmt.Printf("Result len: %v, err: %v\n", len(result), err)
// }
