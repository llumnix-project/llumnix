...
def _create_bench_client_session():
    ...
    BENCH_AIOHTTP_TIMEOUT_SECONDS = 10 * 60  # 10 mins
...
class RequestMonitor:
    def __init__(self, idx: int, monitor: Optional["RealtimeMonitor"]):
        self.idx = idx
        self.monitor = monitor
        self.ttft: Optional[float] = None
        self.is_decoding = False
        self.generates = asyncio.Queue()
        self.success = True
        self.decoding_stat_start_time = 0
        self.decoding_stat_end_time = 0

    def on_first_token(self, ttft: float):
        self.ttft = ttft

    async def on_generate(self, itl: float, num_token: int, decode_step: int):
        if not self.is_decoding:
            self.decoding_stat_start_time = time.time()
            self.is_decoding = True
        await self.generates.put((itl, num_token, decode_step))

    def on_finish(self, success: bool):
        self.decoding_stat_end_time = time.time()
    ...
    def collect(self) -> Tuple[Optional[float], List[float], Optional[float], int]:
        ...
        total_token = 0
        num_tokens_debug = []
        total_token_from_d_step = 0
        for _ in range(self.generates.qsize()):
            itl, num_token, d_step = self.generates.get_nowait()
            itls.append(itl)
            total_token += num_token
            total_token_from_d_step += d_step
            num_tokens_debug.append(num_token)

        tpot: Optional[float] = None

        if self.is_decoding and (total_token or total_token_from_d_step):
            assert self.decoding_stat_start_time > 0
            stat_end_time = self.decoding_stat_end_time or now
            if total_token:
...
class RealtimeMonitor:
    def __init__(self):
        self.req_idx: int = 0
        self.requests: dict[int, RequestMonitor] = {}
        self.closing: dict[int, RequestMonitor] = {}
    ...
    def close_request(self, idx: int):
        assert idx in self.requests
        m = self.requests.pop(idx)
        self.closing[idx] = m
    ...
    async def _loop(self):
        ...
        def _do_collect(reqeuests, ttfts, itls, tpots) -> int:
            ...
            for idx, m in reqeuests.items():
                ttft, req_itls, tpot, req_total_tokens = m.collect()
        ...
        while not self.stop_event.is_set():
            ...
            ttfts, itls, tpots = [], [], []
            total_tokens = 0
            total_tokens += _do_collect(self.requests, ttfts, itls, tpots)
            total_tokens += _do_collect(self.closing, ttfts, itls, tpots)
            ...
            mean_itl = np.mean(itls or nan) * 1000
            p99_itl = np.percentile(itls or nan, 99) * 1000
            max_itl = np.max(itls or nan) * 1000
            mean_tpot = np.mean(tpots or nan) * 1000
    ...
    async with _create_bench_client_session() as session, req_monitor:
        ...
        try:
            async with session.post(
                ...
                if response.status == 200:
                    async for chunk_bytes in response.content:
                        # print(f"resp:{chunk_bytes.decode("utf-8")}",flush=True)
                        chunk_bytes = chunk_bytes.strip()
                        if not chunk_bytes:
                            continue
                        chunk = remove_prefix(chunk_bytes.decode("utf-8"), "data: ")
                        ...
                        if chunk == "[DONE]":
                            ...
                            if data["choices"] and data["choices"][0]["text"]:
                                ...
                                if ttft == 0.0:
                                    ...
                                    output.ttft = ttft
                                    req_monitor.on_first_token(ttft)
...
def sample_qwen_prompt_txt_requests(
    ...
    prompt_contents = open(dataset_path, "r", encoding="utf-8").readlines()
    dataset = []
...
def log_results(
    ...
    if args.output_file:
        ...
        result_dir = f"{LOG_DIR}/benchmark_results"
...
async def benchmark(
    ...
    request_monitor = RealtimeMonitor()
    ...
    result, result_details = log_results(
        benchmark_duration,
        input_requests,
        outputs,
        tokenizer,
...
def run_benchmark(args_: argparse.Namespace):
    ...
    show_request_stats(input_requests)
...
if __name__ == "__main__":
    ...
    group.add_argument(
        ...
        type=float,
        default=0.0,
        help="If setting trim_ratio > 0 is set, an additional metrics table will be displayed. In this table, the first and last trim_ratio × max_concurrency samples are EXCLUDED when calculating the metrics, so as to eliminate the request “bubbles” that occur at bench serving startup and shutdown.",
    )
