...
def check_count_res(responses:List[Any]):
    ...
    assert valid_count == len(responses), f"{error_count} responses failed counting validation."
...
def dump_res(results: List[Any], dataset: str, acc: float, num_questions: int):
    ...
    with open(dump_output_filename, "w") as f:
...
def test_migration_correctness(setup_services, test_config):
    ...
    def send_single_request(connector_type, prompt, dataset):
        ...
        if connector_type == "HybridConnector" and not test_config.get('enable_pd', False):
    ...
    if dataset == 'gsm8k':
        ...
        prompts = ["Please count from 0 to 100 one by one in your response, with each number on a new line."] * num_requests
        labels = None
    else:
        raise ValueError(f"Unknown dataset: {dataset}")
    with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
        futures = [executor.submit(send_single_request, test_config.get('connector_type'), prompt, dataset) for prompt in prompts]
        results = [future.result() for future in concurrent.futures.as_completed(futures)]
    end_time = time.time()

    assert len(results) == num_requests, f"Only {len(results)}/{num_requests} requests completed"
    acc = check_migration_res(dataset=test_config.get('dataset'), results=results, labels=labels)
