...
def prepare_dataset():
    ...
    if not os.path.exists(DATASET_PATH):
        ...
        dataset_dir = snapshot_download(DATASET_NAME, cache_dir='/datasets', repo_type='dataset')
...
def generate_benchmark_config():
    base_configs = [
        {'policy': 'load-balance', 'enable_full_mode_scheduling': True, 'connector_type': 'HybridConnector'},
        {'policy': 'load-balance', 'enable_full_mode_scheduling': False, 'connector_type': 'HybridConnector'},
    ]
...
def test_benchmark(setup_services, test_config, prepare_dataset):
    ...
    assert result['completed'] > 0, "Benchmark run successfully but completed 0 requests"
